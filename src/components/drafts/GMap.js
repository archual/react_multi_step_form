...
export class GMap extends React.Component {
  ...
  handleClick = (marker, type, points) => {
    if (type === "cluster") {
      ...
      for (zoom; zoom < 16; zoom++) {
        mapProps.zoom = zoom;
        let clusters = superCluster(points, {
  ...
  _doDraftsHaveTheSameLength = (first, second) => {
    return this._getDraftStatuses().every(status => {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      !this._doFiltersAreEqual(filter, nextProps.map.filter) ||
      !this._doDraftsHaveTheSameLength(drafts, nextProps.map.drafts)
    ) {
      // Calculate clusters.
    ...
    if (
      nextProps.map.drafts.new.length &&
      ...
      if (
        this.state.newDraftCoords !== false &&
        !nextProps.map.drafts.new.length
      ) {
        this.setState({
          newDraftCoords: false
  ...
  _getClusters(drafts, filter, mapProps) {
    ...
    for (let status in drafts) {
      if (
        (!filter.length || filter.indexOf(status) !== -1) &&
        status !== "new"
      ) {
        let currentDrafts = [];
        currentDrafts.push(
          drafts[status].length ? drafts[status].filter(e => e.editable) : []
        );
        currentDrafts.push(
          drafts[status].length ? drafts[status].filter(e => !e.editable) : []
        );
        ...
        let clustersArray = currentDrafts.map((drafts, index) =>
          mapProps.bounds
            ? superCluster(drafts, {
                ...
                maxZoom: 8, // max zoom level to cluster the points on
                radius: 100 // cluster radius in pixels
              })(mapProps).map(({ wx, wy, numPoints, points, zoom }) => ({
                lat: wy,
                lng: wx,
                text: numPoints,
                numPoints,
  ...
  _distanceToMouse = (markerPos, mousePos, markerProps) => {
    const MARKER_WIDTH =
      34 +
      ((markerProps.text + "").length > 2 && markerProps.status !== "new"
        ? ((markerProps.text + "").length - 2) * 11
        : 0);
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return (
      nextProps.map.drafts.claimed.length !==
        this.props.map.drafts.claimed.length ||
      ...
      (this.props.map.drafts.new.length &&
        (nextProps.map.drafts.new[0].lat !== this.props.map.drafts.new[0].lat ||
          nextProps.map.drafts.new[0].lng !==
            this.props.map.drafts.new[0].lng)) ||
  ...
  render() {
    ...
    return (
      <div onKeyPress={this.handleMarkerKeyPress}>
        <GoogleMapReact
          style={{
            position: "relative",
            margin: 0,
...
GMap.propTypes = {
  ...
  selectedDraft: PropTypes.oneOfType([PropTypes.bool, PropTypes.object]),
